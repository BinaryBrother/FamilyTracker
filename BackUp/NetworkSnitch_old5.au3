...
While 1
	$Target_IP = "192.168.254.107" ;I identified my Pixel 6 from the ARP Table.
	If RunWait("ping.exe -n 1 " & 127.0.0.1, "", @SW_HIDE) == 0 Then; success
		;Do successful
		else
		;not successful
		endif
	$Ping = Ping($Target_IP, 4000)
	If $Ping = 0 Then
		MsgBox($MB_OK, "Network Snitch", "User left network!")
	EndIf
WEnd
